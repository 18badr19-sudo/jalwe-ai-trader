...
def get_control_keyboard():
    ...
    btn_errors = KeyboardButton("⚠️ فحص الأخطاء والنظام")
...
def handle_control_buttons(message):
    ...
    if "تشغيل البوت المتعلم" in text:
        ...
        error_report = (
            ...
            f"• **حالة الخطأ الحالي:**\n`{last_error}`\n\n"
            f"• **حالة اتصال تيليجرام:** `مستقر (Long Polling نشط)`\n"
    ...
    try:
        ...
        for p in positions:
            ...
            if pnl_pct >= TAKE_PROFIT_PCT:
                ...
                bot.send_message(TELEGRAM_CHAT_ID, f"🎯💰 **تحقيق هدف الربح**\n📌 السهم: `{symbol}`\n📈 الربح: `+{pnl_pct*100:.2f}%`", parse_mode="Markdown")
            elif pnl_pct <= -STOP_LOSS_PCT:
                alpaca.close_position(symbol)
                bot.send_message(TELEGRAM_CHAT_ID, f"🛡️🛑 **وقف الخسارة**\n📌 السهم: `{symbol}`\n📉 الخسارة: `{pnl_pct*100:.2f}%`", parse_mode="Markdown")
        ...
        for symbol in watchlist:
            ...
            if signal == 1 and symbol not in active_symbols and cash > 30:
                ...
                bot.send_message(TELEGRAM_CHAT_ID, f"🚨🤖 **شراء ذكي مؤكد**\n📌 السهم: `{symbol}`", parse_mode="Markdown")
            elif signal == 0 and symbol in active_symbols:
                alpaca.close_position(symbol)
                bot.send_message(TELEGRAM_CHAT_ID, f"🔄🧠 **إغلاق الصفقة لتغير المؤشرات**\n📌 السهم: `{symbol}`", parse_mode="Markdown")
    except Exception as e:
        last_error = str(e)
...
if __name__ == "__main__":
    print("INFO - JALWE AI Ultimate Edition is running...")
    try:
        bot.remove_webhook()
        time.sleep(3)
    except Exception as e:
