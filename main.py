...
# إيقاف أي ويب هوك سابق لمنع تداخل الجلسات
try:
    bot.remove_webhook()
    time.sleep(1)
except Exception:
...
bot_running = True
last_error = "النظام يعمل بكفاءة تامة (الإصدار الشامل الذكي) 🚀🛡️"

# إعدادات المخاطر
TAKE_PROFIT_PCT = 0.03  # جني الأرباح 3%
STOP_LOSS_PCT = 0.02    # وقف الخسارة 2%

# قائمة المراقبة القيادية الأساسية + رادار السيولة
...
def advanced_market_scanner():
    try:
        ...
        import random
        random_sample = random.sample(tradable, min(15, len(tradable)))
        
        hot_stocks = list(set(CORE_WATCHLIST + random_sample))
        return hot_stocks
...
def send_status_report(chat_id):
    ...
    try:
        ...
        state_text = "🟢 النظام يعمل بأقصى طاقة (الذكاء + الرادار + الحماية)" if bot_running else "🛑 متوقف مؤقتاً"
        ...
        report = (
            ...
            f"• **الصفقات المفتوحة:** `{pos_count} صفقة`\n"
            f"• **أهداف الحماية:** جني أرباح `+{TAKE_PROFIT_PCT*100}%` | وقف خسارة `-{STOP_LOSS_PCT*100}%`\n"
            f"• **سجل النظام:**\n`{last_error}`"
        )
...
def handle_control_buttons(message):
    ...
    if "تشغيل البوت المتعلم" in text:
        bot_running = True
        bot.send_message(chat_id, "🟢 **تم تفعيل النسخة الشاملة لـ JALWE AI بنجاح.**", reply_markup=get_control_keyboard())
...
# ==================== 2. عقل الذكاء الاصطناعي المتقدم (مؤشرات متقدمة) ====================
def ml_predict_signal(symbol):
    try:
        ...
        # مؤشر البولنجر باند (Bollinger Bands) والتقلب
...
# ==================== 3. دورة الفحص الشاملة والتداول وإدارة المخاطر ====================
def ai_learning_trading_cycle():
    ...
    try:
        account = alpaca.get_account()
        cash = float(account.cash)
        last_error = "النظام يعمل بذكاء وحماية تامة ✅"
        
        # أولاً: فحص الصفقات المفتوحة وتطبيق وقف الخسارة وجني الأرباح
        positions = alpaca.list_positions()
        for p in positions:
            symbol = p.symbol
        ...
        # ثانياً: فحص السوق واصطياد الفرص عبر القائمة الموسعة والذكاء الاصطناعي
        ...
        for symbol in watchlist:
            ...
            if signal == 1 and symbol not in active_symbols and cash > 30:
                ...
                if TELEGRAM_CHAT_ID:
                    bot.send_message(TELEGRAM_CHAT_ID, f"🚨🤖 **JALWE AI (شراء ذكي جديد)**\n📌 السهم: `{symbol}`\n💡 إشارة مؤكدة من نموذج الذكاء الاصطناعي والمؤشرات المتقدمة!", parse_mode="Markdown")
            elif signal == 0 and symbol in active_symbols:
                alpaca.close_position(symbol)
                if TELEGRAM_CHAT_ID:
                    bot.send_message(TELEGRAM_CHAT_ID, f"🔄🧠 **JALWE AI (إغلاق الصفقة لتغير الاتجاه الفني)**\n📌 السهم: `{symbol}`", parse_mode="Markdown")
    except Exception as e:
        last_error = str(e)
...
if __name__ == "__main__":
    print("INFO - JALWE AI Ultimate Edition is online 24/7...")
    
    # خيط مستقل لتنفيذ الجدول الزمني بدون تداخل
    import threading
    def schedule_thread():
        while True:
    ...
    # تشغيل البوت الأساسي باستقرار تامة
    while True:
        try:
